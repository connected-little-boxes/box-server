...
const stages = {
  ...
  StatusLedCheck: {
    description: ["*Checking the status led",
  ...
  FlashDone: {
    description: ["*Software load complete",
      ...
      `You can now go on to configure the settings in the device.`,
      `If you are having problems with your device you can use the light on it to diagnose problems. `
    ],
    inputFields: [],
    buttons: [
      { buttonText: "Configure settings", buttonDest:  async () => { window.location.replace("/connect/usb"); }},
      { buttonText: "Diagnose problems", buttonDest:  async () => { await selectStage(stages.StatusLedCheck); }},
      { buttonText: "Exit flash", buttonDest: doExitFlash }
    ]
  },
  ConnectFailed: {
